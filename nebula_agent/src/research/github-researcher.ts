...
export class GithubResearcher {
    ...
    async searchForCandidates(): Promise<GithubCandidate[]> {
        if (!this.token) {
            await logger.warn('github_search', 'GITHUB_TOKEN is missing. Skipping search.');
            return [];
        }
        ...
        await logger.info('github_search', `Found ${rawCandidates.length} raw candidates, filtered to ${filtered.length}`, {
    ...
    private async fetchRecentActiveRepos(): Promise<any[]> {
        ...
        try {
            ...
            if (!res.ok) {
                await logger.error('github_api_error', `API returned ${res.status}`);
                return [];
            }
            ...
            await logger.error('github_fetch_fail', e.message);
            return [];
        }
    }

    private deterministicFilter(repos: any[]): GithubCandidate[] {
        // Implement rules:
        // - No forks
