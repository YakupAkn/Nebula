...
exports.RateLimitManager = exports.SafetyLimitReachedError = exports.RateLimitHitError = void 0;
const db_1 = require("../storage/db");
const logger_1 = require("../utils/logger");
class RateLimitHitError extends Error {
    retryAfterS;
...
exports.RateLimitHitError = RateLimitHitError;
class SafetyLimitReachedError extends Error {
    constructor(msg) {
        super(msg);
        this.name = 'SafetyLimitReachedError';
    }
}
exports.SafetyLimitReachedError = SafetyLimitReachedError;
class RateLimitManager {
    static getDailyMax() {
        return parseInt(process.env.AGENT_MAX_AI_REQUESTS_PER_DAY || '50', 10);
    }
    static async reserveRequest() {
        ...
        if (error && error.code === 'PGRST116') { // not found
            // Create record
            const { data: inserted, error: insertError } = await db_1.db
                .from('agent_usage_daily')
        ...
        if (data && data.ai_requests >= max) {
            await logger_1.logger.warn('safety_limit', `Reached local safety limit: ${data.ai_requests}/${max} today.`);
    ...
    static async recordSuccess(inputTokens = 0, outputTokens = 0) {
        const today = new Date().toISOString().split('T')[0];
        ...
        const { data } = await db_1.db.from('agent_usage_daily').select('successful_requests, input_tokens, output_tokens').eq('usage_date', today).single();
        if (data) {
            await db_1.db.from('agent_usage_daily')
                .update({
    ...
    static async recordFailure(wasRateLimit) {
        const today = new Date().toISOString().split('T')[0];
        const { data } = await db_1.db.from('agent_usage_daily').select('failed_requests, rate_limit_hits').eq('usage_date', today).single();
        if (data) {
            await db_1.db.from('agent_usage_daily')
                .update({
